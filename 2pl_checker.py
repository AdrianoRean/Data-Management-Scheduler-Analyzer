...
'''
def clean_transaction_involved(transaction):
    for key in list(transactions_involved.keys()):
        transactions_involved[key] = [
            (other_tr, other_act) for (other_tr, other_act) in transactions_involved[key] if other_tr != transaction
        ]
        # Rimuovere risorse vuote
        if not transactions_involved[key]:
            del transactions_involved[key]

def check_if_lock_available(transaction, action, resource, index, loop=set()):
    if index == 0:
        return True
    ...
    if transaction == previous_transaction:
        return True
    # Controllo ricorsivo per le dipendenze
    for other_resource, other_action in resources_needed[previous_transaction]:
        other_index = transactions_involved[other_resource].index((previous_transaction, other_action))
        if previous_transaction in loop:
            return False
        loop.add(previous_transaction)
        if not check_if_lock_available(previous_transaction, other_action, other_resource, other_index, loop):
            return False
    return True

def two_pl_checker(schedule):
    # Inizializzazione
    for action, tr, resource in schedule:
        resources_needed.setdefault(tr, []).append((resource, action))
        transactions_involved.setdefault(resource, []).append((tr, action))

    # Check e lock
    for action, tr, resource in schedule:
        index = transactions_involved[resource].index((tr, action))
        if not check_if_lock_available(tr, action, resource, index, loop={tr}):
            return False
        resources_needed[tr].remove((resource, action))
        if not resources_needed[tr]:
            clean_transaction_involved(tr)
    return True
